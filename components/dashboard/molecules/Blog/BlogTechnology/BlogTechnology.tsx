...
export const BlogTechnology: FC<BlogTechnologyProps> = () => {
     ...
     return (
          <div className="flex flex-col items-start gap-4 lg:flex-row lg:gap-16">
               ...
               <div className="grid grid-cols-3 gap-2 lg:grid-cols-5 lg:gap-4 lg:gap-x-16 xl:grid-cols-7">
                    {Array.isArray(matchedTechnologies)
                         ? matchedTechnologies.map((tech, idx) => (
                                <div
                                     key={`${tech?.name}-${idx}`}
                                     className="flex w-full items-center gap-2 overflow-hidden rounded-full border border-[#E6E6E6] px-4 py-2"
                                >
                                     {tech?.icon ? (
